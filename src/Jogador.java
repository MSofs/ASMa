                ...
                if (result.length > 0) {
                    ...
                    if (msg != null) {
                        ...
                        if (msg.getPerformative() == ACLMessage.INFORM) { // movimento agente conforme estrategia
                            try {
                                //System.out.println(myAgent.getLocalName() + ": Movimentou-se");
                                ...
                                if (e.getTipo() == 1) { // se receber uma posicao para onde ir
                                    ...
                                    Position novaPosicao = ShortestPath.move(e, current_location.getPosition());

                                    System.out.println( myAgent.getLocalName() + "suposto ir para " + e.getPosition() + "Antiga " + current_location.getPosition() + "¬¬¬ Nova "+ novaPosicao);
                                 ...
                                 //   int y = current_location.getPosition().getY();
                                    ...
                                    //System.out.println("POS JOG: " + current_location);
                                    //System.out.println("POS INIMIGOS: " + e.getPos_Inimigos());
                                    //&& (!e.getPos_Inimigos().contains(new Position(x+1,y)))
                                    /*
                                    ...
                                    current_location.setPositionX(novaPosicao.getX());
                                    current_location.setPositionY(novaPosicao.getY());

                                    System.out.println(" No Jogador a localizaçao a ser enviada: " + current_location);
                                ...
                                if (e.getTipo() == 0) { // se nao tem inimigos stutter
                                    ...
                                    myAgent.send(mensagem);
                                     //current_location.getPosition().equals(new Position(17,17)
                                    ACLMessage men = new ACLMessage(ACLMessage.CONFIRM);
                                    for (DFAgentDescription dfAgentDescription : result) {
                                        men.addReceiver(dfAgentDescription.getName());
                                    }
